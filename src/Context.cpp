...
{
    Context::Context(JavaVM *const jvm):
        jvm_(jvm),
        detach_(true)
    {
        BOOST_ASSERT(jvm_);
    ...
    Context::Context(JNIEnv *const env):
        env_(env),
        detach_(false)
    {
        BOOST_ASSERT(env_);
    ...
    JNIEnv *Context::envNoExcept() noexcept
    {
        return env_;
    }
    ...
    void Context::throwIfOccured()
    {
        if (env_->ExceptionCheck())
            BOOST_THROW_EXCEPTION(JavaError());
    }

    void Context::throwNew(const char *const className,
    ...
    void Context::throwNewFromEnv(JNIEnv *env,
