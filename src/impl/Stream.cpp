...
void Java_com_yandex_contest_invoker_impl_Stream_classInit(JNIEnv *env,
                                                           jclass streamClass) {
...
void Java_com_yandex_contest_invoker_impl_Stream_finalize(JNIEnv *env,
                                                          jobject self) {
...
struct GetAccessModeVisitor : boost::static_visitor<AccessMode> {
  AccessMode operator()(const File &file) const { return file.accessMode; }
...
jobject Java_com_yandex_contest_invoker_impl_Stream_getAccessMode(
    JNIEnv *env, jobject self) {
  YANDEX_JNI_METHOD_BEGIN_THIS(env, streamClass_, self)
  const AccessMode accessMode =
      boost::apply_visitor(GetAccessModeVisitor(), *this_);
...
struct ToStringVisitor : boost::static_visitor<std::string> {
  ...
  std::string operator()(const Pipe::End &pipeEnd) const {
    return str(boost::format("Pipe::End: %1% %2%") % pipeEnd.pipeId %
               pipeEnd.end);
  }
...
jstring Java_com_yandex_contest_invoker_impl_Stream_toString(JNIEnv *env,
                                                             jobject self) {
  YANDEX_JNI_METHOD_BEGIN_THIS(env, streamClass_, self)
  return newStringUTF(boost::apply_visitor(ToStringVisitor(), *this_))
      .release();
  YANDEX_JNI_METHOD_END_OBJECT(env)
}
