...
jobjectArray Java_com_yandex_contest_invoker_impl_PackageInit_getClasses(
    JNIEnv *env, jclass) {
  ...
  LocalRef<jobjectArray> classes(
      ctx->env()->NewObjectArray(com_yandex_contest_invoker_impl_classNamesSize,
                                 classClass.get(), nullptr));
  for (std::size_t i = 0; i < com_yandex_contest_invoker_impl_classNamesSize;
       ++i) {
