...
void Java_com_yandex_contest_invoker_impl_process_1group_ProcessGroupResult_classInit(
  ...
  completionStatusClass_.assign(
      "com/yandex/contest/invoker/process_group/"
      "IProcessGroupResult$CompletionStatus");
...
jobject
Java_com_yandex_contest_invoker_impl_process_1group_ProcessGroupResult_getCompletionStatus(
    JNIEnv *env, jobject self) {
...
jobject
Java_com_yandex_contest_invoker_impl_process_1group_ProcessGroupResult_getResourceUsage(
    JNIEnv *env, jobject self) {
  YANDEX_JNI_METHOD_BEGIN_THIS(env, processGroupResultClass_, self)
  return impl::process_group::resource_usage::create(this_->resourceUsage)
      .release();
  YANDEX_JNI_METHOD_END_OBJECT(env)
}
