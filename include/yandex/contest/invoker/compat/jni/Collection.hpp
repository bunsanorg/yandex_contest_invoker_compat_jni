...
LocalRef<jobject> newCollection(const std::string &clazz,
  ...
  const jmethodID ctorId =
      ctx->env()->GetMethodID(collectionClass.get(), "<init>", "()V");
  const jmethodID addId = ctx->env()->GetMethodID(collectionClass.get(), "add",
                                                  "(Ljava/lang/Object;)Z");
...
LocalRef<jobject> newMap(const std::string &clazz, const Iterable &iterable,
  ...
  const jmethodID ctorId =
      ctx->env()->GetMethodID(mapClass.get(), "<init>", "()V");
