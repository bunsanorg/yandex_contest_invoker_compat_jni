...
template <bool IsObject, bool IsEnum, typename CType, typename JType,
...
template <bool IsObject, bool IsEnum, typename CType, typename JType,
          JType (JNIEnv::*ENVGet)(jobject, jmethodID, ...)>
constexpr JType (JNIEnv::*basic_info<IsObject, IsEnum, CType, JType,
                                     ENVGet>::envget)(jobject, jmethodID, ...);
...
template <typename T>
struct optional_info<
    T, typename std::enable_if<!info<T>::is_primitive, void>::type>
    : basic_info<info<T>::is_object, info<T>::is_enum, boost::optional<T>,
                 typename info<T>::jtype, &JNIEnv::CallObjectMethod> {
  static std::string jclass() { return info<T>::jclass(); }
};

template <typename T>
struct optional_info<T,
                     typename std::enable_if<info<T>::is_primitive, void>::type>
    : basic_info<true, info<T>::is_enum, boost::optional<T>, jobject,
...
template <typename CType, typename JType,
          JType (JNIEnv::*ENVGet)(jobject, jmethodID, ...)>
struct basic_primitive_info : basic_info<false, false, CType, JType, ENVGet> {
...
struct info<std::chrono::duration<Rep, Period>>
    : basic_primitive_info<std::chrono::duration<Rep, Period>, jlong,
...
struct info<ContainerConfig>
    : invoker_object_info<ContainerConfig, info<ContainerConfig>> {
  static std::string jclassbase() { return "IContainerConfig"; }
};

template <>
struct info<ControlProcessConfig>
    : invoker_object_info<ControlProcessConfig, info<ControlProcessConfig>> {
  static std::string jclassbase() { return "IControlProcessConfig"; }
};

template <>
struct info<invoker::lxc::Config>
    : invoker_object_info<invoker::lxc::Config, info<invoker::lxc::Config>,
                          false> {
...
struct info<invoker::lxc::MountConfig>
    : invoker_object_info<invoker::lxc::MountConfig,
                          info<invoker::lxc::MountConfig>, false> {
...
struct info<invoker::lxc::RootfsConfig>
    : invoker_object_info<invoker::lxc::RootfsConfig,
                          info<invoker::lxc::RootfsConfig>, false> {
  static std::string jclassbase() { return "lxc/IRootfsConfig"; }
};

template <>
struct info<filesystem::Config>
    : invoker_object_info<filesystem::Config, info<filesystem::Config>> {
...
struct info<filesystem::File>
    : invoker_object_info<filesystem::File, info<filesystem::File>> {
...
struct info<filesystem::Directory>
    : invoker_object_info<filesystem::Directory, info<filesystem::Directory>> {
  static std::string jclassbase() { return "filesystem/IDirectory"; }
};

template <>
struct info<filesystem::Device>
    : invoker_object_info<filesystem::Device, info<filesystem::Device>> {
...
struct info<filesystem::SymLink>
    : invoker_object_info<filesystem::SymLink, info<filesystem::SymLink>> {
  static std::string jclassbase() { return "filesystem/ISymLink"; }
};

template <>
struct info<filesystem::Fifo>
    : invoker_object_info<filesystem::Fifo, info<filesystem::Fifo>> {
...
struct info<system::unistd::MountEntry>
    : invoker_object_info<system::unistd::MountEntry,
                          info<system::unistd::MountEntry>, false> {
  ...
  static std::string convert(const std::string &name) {
    if (name == "lxc") {
      return "LxcConfig";
               ...
               name != "processDefaultSettings" && name != "containersDir") {
      return name + "Config";
    }
