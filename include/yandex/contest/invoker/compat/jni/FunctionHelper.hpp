...
#define YANDEX_JNI_METHOD_END_VOID(ENV)                            \
  ...
  catch (::yandex::contest::invoker::compat::jni::JavaError &      \
         e) { /*does nothing, exception was already thrown*/       \
  }                                                                \
  /*here we map local exception classes to Java*/                  \
  YANDEX_JNI_METHOD_MAP_ERROR(                                     \
      ENV, ::yandex::contest::invoker::ContainerIllegalStateError, \
      "java/lang/IllegalStateException")                           \
  YANDEX_JNI_METHOD_MAP_ERROR(ENV, std::exception,                 \
